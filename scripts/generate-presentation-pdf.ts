...
<body>
  ...
  <div class="callout orange">
    <div class="callout-title">The National Urgency (Why BIS Compliance is Broken)</div>
    India currently enforces over <strong>20,000 active Indian Standards</strong> and <strong>700+ mandatory Quality Control Orders (QCOs)</strong>. Under <strong>Section 16 and 29 of the BIS Act, 2016</strong>, producing or selling non-certified goods carries severe criminal penalties, asset seizures, and heavy fines. For over <strong>63 million MSMEs</strong>, deciphering dense gazette notifications is prohibitively expensive, while generic AI models (ChatGPT/Claude) hallucinate invalid standard designations (~35% error rate).
  </div>

  <h2 class="section-heading"><span class="badge">KEY INNOVATION</span> The 5 Pillars of Innovation in Manak AI</h2>
  
  <div class="pillar-grid">
    <div class="pillar-card">
      <div class="pillar-title">1. Clause-Level Anti-Hallucination Verifier</div>
      <div class="pillar-problem"><strong>Problem:</strong> Generic LLMs invent fake clauses (e.g. "per Clause 7.2") causing audit rejections.</div>
      <div class="pillar-solution"><strong>Innovation:</strong> Structural chunking with clause-level metadata (Standard, Clause, Title, Page). A post-generation verification layer interceptively cross-references every <code>[REF_N]</code> tag against retrieved evidence, stripping unverified tags before display.</div>
    </div>

    <div class="pillar-card accent">
      <div class="pillar-title">2. Cross-Lingual Semantic Bridge (Hindi → English)</div>
      <div class="pillar-problem"><strong>Problem:</strong> Indian artisans query in Hindi, but 100% of BIS standards are drafted in technical English.</div>
      <div class="pillar-solution"><strong>Innovation:</strong> Cross-Lingual Entity Expander maps vernacular trade terms to English standard taxonomy, running a Dual-Index Hybrid Search (pgvector 768-dim + BM25 keyword matching) using Reciprocal Rank Fusion.</div>
    </div>

    <div class="pillar-card green">
      <div class="pillar-title">3. Script-Preserving Devanagari Normalizer</div>
      <div class="pillar-problem"><strong>Problem:</strong> Machine translators convert "IS 14543" to "आई एस १४५४३", breaking search in official BIS portals.</div>
      <div class="pillar-solution"><strong>Innovation:</strong> Custom regex normalization engine ensures all standard designations, alloy grades, and CM/L license codes remain strictly in <strong>Latin alphanumeric script</strong>, even in 100% fluent Hindi text.</div>
    </div>

    <div class="pillar-card">
      <div class="pillar-title">4. Statutory QCO Disambiguation Engine</div>
      <div class="pillar-problem"><strong>Problem:</strong> Only ~700 of 20,000 standards are mandatory; generic models cannot distinguish laws from guidelines.</div>
      <div class="pillar-solution"><strong>Innovation:</strong> Direct mapping between Indian Standards and Ministry Gazette Orders (DPIIT/Consumer Affairs), tagging products with dynamic badges: 🔴 <strong>MANDATORY (Scheme-I)</strong> or 🟢 <strong>VOLUNTARY</strong>.</div>
    </div>
  </div>
  ...
  <div class="timeline-item">
    ...
    <div class="timeline-content">
      <strong>The Technical Architecture</strong>
  ...
  <div class="qa-box">
    <div class="qa-question">Q1: "What if an LLM hallucinates a clause number or creates a non-existent standard?"</div>
    <div class="qa-answer"><strong>Answer:</strong> "We don't trust raw LLM output. In <code>src/lib/rag/citations.ts</code>, we run a post-generation citation validation layer. Every <code>[REF_N]</code> tag is matched against the exact retrieved chunks in the Evidence Block. If the LLM generates a clause not in the retrieved evidence, the tag is stripped and a hallucination alert is triggered. In our automated evaluation benchmark of 100+ regulatory queries, Manak AI achieved <strong>0.0% Hallucinations</strong>."</div>
  </div>
